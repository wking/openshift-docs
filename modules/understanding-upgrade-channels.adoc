...
You can also configure an empty channel to tell the Cluster Version Operator to not fetch available updates from the upgrade recommendation service (for example, because your cluster has restricted network access and there is no local, reachable upgrade recommendation service).  You can configure an empty channel with the `oc adm upgrade channel` command from the OpenShift CLI.
